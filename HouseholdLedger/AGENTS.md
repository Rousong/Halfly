...
此目录存放 iOS App 的 SwiftUI 源码和应用资源，按 App、Features、Core、Shared、Resources 分层组织。当前负责多账本数据模型、SwiftData 持久化、账本创建与切换、支出录入、列表、双人账本概览、双人账本结清界面和 App 图标资源。
...
│   └── RootView.swift # 应用根视图，提供账本管理、新建账本入口，并组装账本、记录、列表、概览和结清五个 Tab。
├── Core/ # 核心业务模型和 SwiftData 基础设施目录。
│   ├── AGENTS.md # Core 目录说明文档，记录目录规则、代码简介和结构。
│   ├── Models/ # 账本领域模型目录。
│   │   ├── AGENTS.md # Models 目录说明文档，记录目录规则、代码简介和结构。
│   │   └── Models.swift # SwiftData 账本、支出、结清、筛选条件和双人 AA 汇总模型定义。
│   └── Persistence/ # SwiftData 持久化和状态管理目录。
│       ├── AGENTS.md # Persistence 目录说明文档，记录目录规则、代码简介和结构。
│       └── LedgerStore.swift # 多账本状态管理、当前账本切换、SwiftData 读写和双人结清计算逻辑。
├── Features/ # 用户可见业务功能目录。
│   ├── AGENTS.md # Features 目录说明文档，记录目录规则、代码简介和结构。
│   ├── Expenses/ # 基于当前账本的支出录入和支出历史功能目录。
│   │   ├── AGENTS.md # Expenses 目录说明文档，记录目录规则、代码简介和结构。
│   │   └── Views/ # 支出功能页面目录。
│   │       ├── AGENTS.md # Views 目录说明文档，记录目录规则、代码简介和结构。
│   │       ├── AddExpenseView.swift # 支出录入页面，基于当前账本成员选择支付者并处理金额、日期、类别和备注输入。
│   │       └── ExpenseListView.swift # 支出列表页面，按当前账本和筛选条件展示记录并支持删除。
│   ├── Overview/ # 双人账本概览功能目录。
│   │   ├── AGENTS.md # Overview 目录说明文档，记录目录规则、代码简介和结构。
│   │   └── Views/ # 概览功能页面目录。
│   │       ├── AGENTS.md # Views 目录说明文档，记录目录规则、代码简介和结构。
│   │       └── OverviewView.swift # 概览页面，在双人账本中展示 AA 汇总、结清状态、未结清支出和最近结清记录。
│   └── Settlements/ # 双人账本 AA 结清功能目录。
...
└── Shared/ # 跨功能复用代码目录。
    ...
    └── Utilities/ # 通用工具函数目录。
        ...
        └── LedgerFormatters.swift # 金额、日期和动态参与人结清转账文案格式化函数。
```
