...
struct AddExpenseView: View {
    ...
    var body: some View {
        NavigationStack {
                ...
                if let currentLedger {
                    Form {
                        ...
                        Section("支出") {
                            ...
                            Picker("支付者", selection: $payerName) {
                                ForEach(currentLedger.participantNames, id: \.self) { item in
                                    Text(item).tag(item)
                                }
                            }
            ...
            .navigationTitle(currentLedger?.name ?? "记录一笔支出")
            ...
            .onAppear(perform: syncPayerSelection)
            .onChange(of: store.selectedLedgerID) { _, _ in
                syncPayerSelection()
            }
        }
    }
    ...
    private func syncPayerSelection() {
        guard let currentLedger else {
            payerName = ""
            return
        }

        if !currentLedger.participantNames.contains(payerName) {
            payerName = currentLedger.participantNames.first ?? ""
        }
    }
}
